<?php
/**
 * Verschlüsselt bestehende Kundeneinträge in der Datenbank.
...
$wanted = get_customer_encrypted_columns();
...
try {
    $stmt = $pdo->query("SELECT id, " . implode(', ', array_map(function ($c) { return '`' . $c . '`'; }, $columns)) . " FROM customers");
    ...
    foreach ($rows as $row) {
        ...
        foreach ($columns as $col) {
            ...
            $encrypted = encrypt_customer_value($val);
            if ($encrypted === $val) {
                continue;
            }
            $updates[] = "`" . $col . "` = ?";
    ...
    if ($isCli) {
        echo "Fertig. Verschlüsselt: $updated Kunden.\n";
    } else {
        header('Content-Type: application/json');
        echo json_encode([
            'success' => true,
            'message' => 'Bestehende Kundendaten verschlüsselt',
            'updated' => $updated
        ]);
    }
} catch (Exception $e) {
    error_log("Kunden verschlüsseln: " . $e->getMessage());
    if ($isCli) {
