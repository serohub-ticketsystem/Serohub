...
$navMobileCompactTitle = 'E-Mail-Präferenzen';
$navMobileCompactBackUrl = BASE_URL . 'settings/index.php?section=notifications';
$navMobileCompactBackLabel = 'Zurück zu Benachrichtigungen';
...
// Kategorien und Benachrichtigungstypen (gleiche wie in notification-settings.php)
    ...
    <main class="mx-4 mt-0 max-lg:mt-0 max-lg:mx-0 max-lg:px-4 lg:h-full">
        <div>
            <div class="grid grid-cols-12 gap-4 bg-gray-50 dark:bg-primary-50 lg:h-full">
                ...
                <aside class="hidden lg:block lg:col-span-3 lg:mx-0 lg:self-start lg:sticky lg:top-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto">
                ...
                <!-- Settings Container -->
                <div class="col-span-full lg:col-span-9 lg:col-start-4 lg:max-h-[calc(100vh-2rem)] lg:overflow-y-auto lg:pe-1 lg:pb-28" id="settingsContainer">
...
function renderSettings() {
    ...
    Object.entries(notificationCategories).forEach(([categoryKey, category]) => {
        // Bestimme aktuelle Einstellung für diese Kategorie (für E-Mail statt System)
        const currentSetting = categorySettings[categoryKey] || 'none';
        const allTypes = Object.keys(category.types);
        const importantTypes = allTypes.filter(type => category.types[type].wichtig);
        
        // Berechne aktive Typen basierend auf Einstellung
        let activeTypes = [];
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-3 lg:mb-4">
                <div class="p-4 sm:p-5 lg:p-6">
                    <div class="mb-4 flex items-start justify-between gap-3">
                        ...
                        <button 
                            class="shrink-0 inline-flex h-10 items-center rounded-lg px-3 text-sm font-medium text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 bg-transparent hover:bg-transparent active:bg-transparent focus:bg-transparent dark:bg-transparent dark:hover:bg-transparent dark:active:bg-transparent dark:focus:bg-transparent focus:outline-none"
                    ...
                    <div class="grid grid-cols-3 gap-2.5" role="group" aria-label="E-Mail-Auswahl ${category.name}">
                        <button type="button"
                            class="inline-flex w-full items-center justify-between rounded-lg border px-3.5 py-3 text-left transition-colors focus:z-10 focus:outline-none focus:ring-2 focus:ring-primary-500 ${currentSetting === 'all' ? 'border-primary-600 dark:border-primary-500' : 'border-gray-300 bg-white hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-gray-700'}"
                            style="${currentSetting === 'all' ? `background-color:${isDark ? '#334155' : '#e5e7eb'};color:${isDark ? '#f8fafc' : '#111827'};` : ''}"
                            onclick="selectCategorySetting('${categoryKey}', 'all')"
                            aria-pressed="${currentSetting === 'all' ? 'true' : 'false'}">
                            <span class="min-w-0">
                                <span class="block text-sm font-semibold ${currentSetting === 'all' ? '' : 'text-gray-900 dark:text-gray-100'}">Alle</span>
                            </span>
                        ...
                        <button type="button"
                            class="inline-flex w-full items-center justify-between rounded-lg border px-3.5 py-3 text-left transition-colors focus:z-10 focus:outline-none focus:ring-2 focus:ring-primary-500 ${currentSetting === 'important' ? 'border-primary-600 dark:border-primary-500' : 'border-gray-300 bg-white hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-gray-700'}"
                            style="${currentSetting === 'important' ? `background-color:${isDark ? '#334155' : '#e5e7eb'};color:${isDark ? '#f8fafc' : '#111827'};` : ''}"
                            onclick="selectCategorySetting('${categoryKey}', 'important')"
                            aria-pressed="${currentSetting === 'important' ? 'true' : 'false'}">
                            <span class="min-w-0">
                                <span class="block text-sm font-semibold ${currentSetting === 'important' ? '' : 'text-gray-900 dark:text-gray-100'}">Wichtige</span>
                            </span>
                        ...
                        <button type="button"
                            class="inline-flex w-full items-center justify-between rounded-lg border px-3.5 py-3 text-left transition-colors focus:z-10 focus:outline-none focus:ring-2 focus:ring-primary-500 ${currentSetting === 'none' ? 'border-primary-600 dark:border-primary-500' : 'border-gray-300 bg-white hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-gray-700'}"
                            style="${currentSetting === 'none' ? `background-color:${isDark ? '#334155' : '#e5e7eb'};color:${isDark ? '#f8fafc' : '#111827'};` : ''}"
                            onclick="selectCategorySetting('${categoryKey}', 'none')"
                            aria-pressed="${currentSetting === 'none' ? 'true' : 'false'}">
                            <span class="min-w-0">
                                <span class="block text-sm font-semibold ${currentSetting === 'none' ? '' : 'text-gray-900 dark:text-gray-100'}">Keine</span>
                            </span>
                ...
                <div id="collapse-${categoryKey}" class="hidden border-t border-gray-200 dark:border-gray-700">
                    <div class="p-4 sm:p-5 lg:p-6 bg-gray-50 dark:bg-gray-900/50">
                        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">
                            Aktive E-Mail-Benachrichtigungen (${activeTypes.length}):
                        </h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-3">
                            ${activeTypes.length > 0 ? activeTypes.map(type => {
                                ...
                                `;
                            }).join('') : '<p class="text-sm text-gray-500 dark:text-gray-400 col-span-full">Keine aktiven E-Mail-Benachrichtigungen</p>'}
                        </div>
                    </div>
                </div>
            </div>
...
function selectCategorySetting(categoryKey, setting) {
    ...
    const oldSetting = categorySettings[categoryKey] || 'none';
    if (oldSetting !== setting) {
        categorySettings[categoryKey] = setting;
        
        // Update alle Typen dieser Kategorie in pendingChanges (für E-Mail statt System)
        ...
        // Setze alle Typen der Kategorie (email statt system)
        allTypes.forEach(type => {
            ...
            pendingChanges[type].email = shouldBeActive;
        });
...
async function loadSettings() {
    try {
        ...
        // Initialisiere categorySettings basierend auf settings (für E-Mail, Standard: 'all')
        Object.entries(notificationCategories).forEach(([categoryKey, category]) => {
            const allTypes = Object.keys(category.types);
            const importantTypes = allTypes.filter(type => category.types[type].wichtig);
            
            // Prüfe E-Mail-Einstellungen für alle Typen (Default: email !== false wenn nicht gesetzt)
            const allActive = allTypes.every(type => {
                const typeSetting = settings[type];
                return typeSetting?.email !== false;
            });
            ...
            if (noneActive) {
                ...
                // Gemischte Einstellungen - versuche das am nächsten kommende zu finden
                ...
                if (activeCount === 0) {
                    categorySettings[categoryKey] = 'none';
                } else if (activeCount === importantTypes.length) {
                    categorySettings[categoryKey] = 'important';
                } else {
                    categorySettings[categoryKey] = 'all'; // Default bei gemischten Einstellungen
                }
            }
        });
...
async function savePendingChanges() {
    ...
    try {
        // Sende für jeden geänderten Typ einen separaten Request (wie in notification-settings.php)
        const promises = Object.entries(changesToSave).map(([type, typeSettings]) => {
            const originalSettings = settings[type] || { system: true, email: true };
            ...
            return fetch(`${getApiBaseUrl()}notifications/api/settings.php`, {
                ...
                body: JSON.stringify({
                    ...
                    system: originalSettings.system, // System-Einstellung beibehalten
                    email: typeSettings.email // Nur E-Mail-Einstellung aktualisieren
                })
            });
        });
        ...
        if (allSuccess) {
            // Erfolgreich gespeichert - aktualisiere settings mit pendingChanges
            Object.entries(changesToSave).forEach(([type, typeSettings]) => {
                if (!settings[type]) {
                    settings[type] = { system: true, email: true };
                }
                settings[type].email = typeSettings.email;
                if (pendingChanges[type] && pendingChanges[type].email === typeSettings.email) {
                    delete pendingChanges[type];
                }
            });
            if (typeof showToast === 'function') {
                showToast('E-Mail-Präferenzen gespeichert', 'success');
            }
        } else {
        ...
        if (typeof showToast === 'function') {
            showToast('Fehler beim Speichern der E-Mail-Präferenzen: ' + error.message, 'error');
        } else {
            alert('Fehler beim Speichern der E-Mail-Präferenzen: ' + error.message);
        }
    } finally {
